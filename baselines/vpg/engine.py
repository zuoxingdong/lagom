...
class Engine(lagom.BaseEngine):        
    def train(self, n=None, **kwargs):
        ...
        logger('return', utils.describe([sum(traj.rewards) for traj in D], axis=-1, repr_indent=1, repr_prefix='\n'))
        
        E = [traj[-1].info['episode'] for traj in D if 'episode' in traj[-1].info]
        logger('online_return', utils.describe([e['return'] for e in E], axis=-1, repr_indent=1, repr_prefix='\n'))
        logger('online_horizon', utils.describe([e['horizon'] for e in E], axis=-1, repr_indent=1, repr_prefix='\n'))
        logger('running_return', utils.describe(self.env.return_queue, axis=-1, repr_indent=1, repr_prefix='\n'))
        logger('running_horizon', utils.describe(self.env.horizon_queue, axis=-1, repr_indent=1, repr_prefix='\n'))
