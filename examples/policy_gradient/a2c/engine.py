...
class Engine(BaseEngine):
    def train(self, n):
        ...
        D_timesteps = sum([segment.T for segment in D])
    ...
    def log_train(self, train_output):
        ...
        logger.log(key='num_segments', val=sum([len(segment.split_transitions) for segment in D]))
        ...
        logger.log(key='average_immediate_reward', val=np.mean(all_immediate_reward))
        logger.log(key='std_immediate_reward', val=np.std(all_immediate_reward))
        logger.log(key='min_immediate_reward', val=np.min(all_immediate_reward))
        logger.log(key='max_immediate_reward', val=np.max(all_immediate_reward))
