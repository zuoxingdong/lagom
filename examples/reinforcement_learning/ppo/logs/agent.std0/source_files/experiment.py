...
config = Config(
    {'cuda': True, 
     ...
     'agent.target_kl': 0.01,  # appropriate KL between new and old policies after an update, for early stopping (Usually small, e.g. 0.01, 0.05)
     
     # only for continuous control
     'env.clip_action': True,  # clip action within valid bound before step()
     'agent.std0': Grid([0.4, 0.6]),  # initial std
